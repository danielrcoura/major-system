...
const filledEntryArb: fc.Arbitrary<DeckEntry> = fc.record({
...
const searchArb: fc.Arbitrary<string> = fc.string({ minLength: 1, maxLength: 5 }).filter((s) => s.trim() !== '');
...
describe('Property 8: Filtro de busca retorna apenas cartões correspondentes', () => {
  it('for any DeckData and search string, all visible cards match by number, persona name, or consonant label; sections with no matches are hidden', () => {
    fc.assert(
      fc.property(deckDataArb, searchArb, (data: DeckData, search: string) => {
        const { container } = render(
          <CardGrid data={data} filter={search} onCardClick={(_num: string) => {}} />
        );
